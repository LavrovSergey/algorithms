...
public:
	...
	void add_good(const string& name)
	{
		goods.push_back(Goods(name));
	}
	string get_name() const
	{
		return this->name;
	...
	friend bool operator==(const Section& first, const Section& second);
	friend bool operator!=(const Section& first, const Section& second);
	friend bool operator<=(const Section& first, const Section& second);
	friend bool operator<(const Section& first, const Section& second);
	friend bool operator>(const Section& first, const Section& second);
...
{
	out << " " << section.name << " { ";
	for (const auto a : section.goods)
	{
		out << a.get_name() + " ";
	}
	out << "}";
...
bool operator==(const Section& first, const Section& second)
{
...
template<>
string RedBlackTree<Section>::getText(treeNode<Section>* x) {
...
string RedBlackTree<T>::getWebGraphviz(string graphName) {
	string graphText = "digraph " + graphName + " {\n";
...
int main()
{
	auto my_tree = new RedBlackTree<Section>({}, true);
	string operation;
	while (true)
	{
		cin >> operation;
		if (operation == "addteam")
		{
			...
			for (int i = 0; i < count; i++)
			{
				string goods_name;
				cin >> goods_name;
				new_section.add_good(goods_name);
			}
			my_tree->insertPersistent(new_section);
		}
		else if (operation == "erasesection")
		{
			string section_name;
			cin >> section_name;
			my_tree->removePersistent(Section(section_name));
		}
		...
		else if (operation == "search")
		{
			string section_name;
			cin >> section_name;
			cout << my_tree->treeSearch(Section(section_name))->key;
		}
